...
const LoginForm = () => {
  ...
  const onSubmit = async (data) => {
    try {
      const res = await fetch('https://healthai-backend-4.onrender.com/login', {
        method: 'POST',
      ...
      if (res.ok && result.success) {
       ...
       router.push(`/dashboard?username=${encodeURIComponent(username)}&email=${encodeURIComponent(email)}`);
      } else {
